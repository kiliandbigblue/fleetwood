...
import { localDefaultBranch, localTrunk } from '../src/worktree.ts';
...
test('the trunk is origin/HEAD wherever there is a remote', async () => {
...
test('a repo with no trunk under any known name says so rather than guessing', async () => {
  ...
  assert.equal(await localTrunk(repo), undefined);
});
